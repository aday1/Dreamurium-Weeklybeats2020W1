...
bool IntersectSphere(vec3 ro, vec3 rd, vec3 pos, float radius, out vec3 intersectPoint)
{
	...
	float b = dot(ro1, rd);
	float c = dot(ro1, ro1) - radius;
	float d = b*b - c;
	
	intersectPoint = ro + rd*(-b - (d));
...
void main(void)
{
	...
	vec3 rd = normalize(p.x*uu + p.y*vv + 16.*9.*0.01*ww);

	vec4 col = Background;

	vec3 origin;
	
	if(IntersectSphere(ro, rd, ExpPosition, Radius + NoiseAmplitude*66.0, origin))
	{
		col = March(origin, rd);
	}
