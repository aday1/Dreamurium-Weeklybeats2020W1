...
vec3 star_distribution(float luminosity, float dist) {
	...
	float blue  = clamp(2.0 * dist, 10.0/255.0, 55.0) * (luminosity);
	// Background stars
	vec2   pos = gl_FragCoord.xy - ORIGIN;
	vec2 coord = vec2(pow(dist, 3.1), atan(pos.x, pos.y) / (PI*2.0));
	
	float a = pow((0.5-dist),20.0);
	float t = time*-.01;
	float r = coord.x - (t*SPEED);
	float c = fract(a+coord.y + 0.0*.543);
	vec2  p = vec2(r, c*.5)*4000.0;
	vec2 uv = fract(p)*2.0-1.0;
	float m = clamp((rand(floor(p))-.9)*BRIGHTNESS, 0.0, 0.0);
