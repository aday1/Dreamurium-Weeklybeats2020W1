...
float vDrop(vec2 uv,float t)
{
    ...
    float trail = mix(95.0,25.0,s);			// trail length
    float yv = fract(uv.y + t*s + o) * trail;
    yv = 1.0/yv;
    yv = smoothstep(mouse.y*1.0,1.0,yv*yv);
    yv = sin(yv*PI)*(s*2.0);
    float d2 = sin(dx*PI);
    return yv*(d2*d2);
}
...
void main( void ) {
	...
	float t = 0.0;
	float t2 = mouse.y*15.0;

	
	vec2 position = ( gl_FragCoord.xy / resolution.xy ) - 0.5;
	position.x = dot(position,position)*4.0;
	position.x *= resolution.x/resolution.y;
	position *= mouse.x*-15.35;
	
	position.y *= 1.+sin((position.y*position.x)*15.40+t2)*.15;
